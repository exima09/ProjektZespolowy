...
export class WorkerService {
    ...
    updateUser = (user, id: number) => this.http.patch(`/api/user/edit/${id}`, user, getHeaders())
        .pipe(catchError(err => this.errorHandler(err)))

    updateWorker = (worker, id: number) => this.http.patch(`/api/worker/edit/${id}`, worker, getHeaders())
        .pipe(catchError(err => this.errorHandler(err)))
